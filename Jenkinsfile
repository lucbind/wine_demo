pipeline {
    agent any 
     environment ('Set Variable database') {
        ...
        compartmentid="""${sh(
                            returnStdout: true,
                            sh '/usr/local/bin/oci search resource free-text-search --text JSON_ATTACK --raw-output --query "data.items[?contains(\"resource-type\", 'AutonomousDatabase')].\"compartment-id\"|[0]"'
                        )}"""
    }   
   stages {
        stage('Clone Git') {
